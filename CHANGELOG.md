...
* Caching is hardwired, everyting and doesn't update. Currently only `Metalove.purge/0` as stopgap measure
...
### Fixed
- trim whitespace around URLs in ID3 parsing, properly recognize an empty string as nil

## [0.2.3] - 2019-06-21

### Added
- add `.iex.exs` file with convenience aliases and fancy prompt for quicker tornaround
...
- properly handle utf16 strings in ID3 tag text content (did split through utf16 characters if two zero bytes where encountered)
...
- `ml.podcast` mix task to parse ID3 headers from mp3 files, extract the chapter information and images.
